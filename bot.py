...
async def find_verification_log(
    ...
    async for message in channel.history(limit=500, oldest_first=False):
        for embed in message.embeds:
            applicant = get_embed_field(embed, "Applicant")
            if applicant and normalized_query in applicant.lower():
...
def clone_verification_embed(source: discord.Embed, query: str) -> discord.Embed:
    ...
    for field_name in ("Applicant", "Decision", "Reviewed by", "Thread", "Screenshot"):
        value = get_embed_field(source, field_name)
        if value:
            embed.add_field(name=field_name, value=value, inline=field_name in {"Decision", "Reviewed by"})
