...
VERIFICATION_LOGS_CHANNEL_ID = required_int_env("VERIFICATION_LOGS_CHANNEL_ID")

VERIFY_BUTTON_ID = "vera:open_verification"
VERIFY_PVP_BUTTON_ID = "vera:verify_pvp"
VERIFY_NOVA_BUTTON_ID = "vera:verify_nova"
VERIFY_GUEST_PASS_BUTTON_ID = "vera:verify_guest_pass"
...
async def post_verification_log(
    ...
    channel = guild.get_channel(VERIFICATION_LOGS_CHANNEL_ID)
    if not isinstance(channel, discord.TextChannel):
        LOGGER.warning("Verification logs channel %s was not found", VERIFICATION_LOGS_CHANNEL_ID)
        return
...
class StaffDecisionView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Verify Guest Pass",
        style=discord.ButtonStyle.secondary,
        custom_id=VERIFY_GUEST_PASS_BUTTON_ID,
    )
    async def verify_guest_pass(
        self,
        interaction: discord.Interaction,
        button: discord.ui.Button,
    ) -> None:
        await self.finish_verification(interaction, GUEST_PASS_ROLE_ID, "Guest Pass")
