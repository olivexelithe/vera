...
FALLN_ROLE_ID = 1474595541022412983
F4LLN_ROLE_ID = 1474597083838414939
FLL1N_ROLE_ID = 1496274605164204083
FRIENDZONE_ROLE_ID = 1530445441118896319
...
GROUP_FALLN_UNDER_18_BUTTON_ID = "vera:group_falln_under_18"
GROUP_FALLN_OVER_18_BUTTON_ID = "vera:group_falln_over_18"
GROUP_F4LLN_UNDER_18_BUTTON_ID = "vera:group_f4lln_under_18"
GROUP_F4LLN_OVER_18_BUTTON_ID = "vera:group_f4lln_over_18"
GROUP_FLL1N_UNDER_18_BUTTON_ID = "vera:group_fll1n_under_18"
GROUP_FLL1N_OVER_18_BUTTON_ID = "vera:group_fll1n_over_18"
GROUP_FRIENDZONE_UNDER_18_BUTTON_ID = "vera:group_friendzone_under_18"
GROUP_FRIENDZONE_OVER_18_BUTTON_ID = "vera:group_friendzone_over_18"
REJECT_BUTTON_ID = "vera:reject"
VERA_PURPLE = discord.Color(0xB072FF)
APPLICANT_ID_RE = re.compile(r"\[(\d{15,25})\]$")
PULL_UP_RE = re.compile(
    r"^vera\s+pull\s+up\s+(.+?)(?:'s|’s)?\s+verification\s+details\s*$",
    re.IGNORECASE,
)
SCREENSHOT_CACHE: dict[int, tuple[str, bytes]] = {}
...
def group_role_map() -> dict[str, int]:
    return {
        "falln": FALLN_ROLE_ID,
        "f4lln": F4LLN_ROLE_ID,
        "fll1n": FLL1N_ROLE_ID,
        "friendzone": FRIENDZONE_ROLE_ID,
    }


def age_role_map() -> dict[str, int]:
    return {
        "under_18": UNDER_18_ROLE_ID,
        "over_18": OVER_18_ROLE_ID,
    }
...
def format_age_label(value: str) -> str:
    return {
        "under_18": "Under 18",
        "over_18": "Over 18",
        "keep": "Keep current",
...
def current_role_name(member: discord.Member, role_ids: dict[str, int]) -> str:
...
def unique_roles(*roles: discord.Role | None) -> list[discord.Role]:
    seen: set[int] = set()
...
async def ensure_staff_member(interaction: discord.Interaction) -> discord.Member | None:
...
async def attachment_to_file(attachment: discord.Attachment) -> discord.File | None:
    try:
        data = await attachment.read(use_cached=True)
    except discord.HTTPException:
        LOGGER.exception("Could not read screenshot attachment %s", attachment.url)
        return None

    return discord.File(io.BytesIO(data), filename=attachment.filename)


async def cache_thread_screenshot(message: discord.Message) -> None:
    if not isinstance(message.channel, discord.Thread):
        return

    applicant_id = applicant_id_from_thread(message.channel)
    if applicant_id is None or message.author.id != applicant_id:
        return
    ...
    for attachment in message.attachments:
        ...
        try:
            SCREENSHOT_CACHE[message.channel.id] = (
                attachment.filename,
                await attachment.read(use_cached=True),
            )
        except discord.HTTPException:
            LOGGER.exception("Could not cache screenshot attachment %s", attachment.url)
        return
...
async def latest_applicant_screenshot_file(
    thread: discord.Thread,
    applicant_id: int,
) -> discord.File | None:
    cached = SCREENSHOT_CACHE.pop(thread.id, None)
    if cached:
    ...
    async for message in thread.history(limit=100, oldest_first=False):
        ...
        for attachment in message.attachments:
            if is_image_attachment(attachment):
                return await attachment_to_file(attachment)
...
async def post_verification_log(
    ...
    file = await latest_applicant_screenshot_file(thread, applicant.id)
    embed = discord.Embed(
...
async def dm_roles_channel(member: discord.Member) -> bool:
    try:
        await member.send(
            "You have been verified in FALLN. Please choose your roles here: "
...
class GroupDecisionView(discord.ui.View):
    ...
    @discord.ui.button(
        label="FALLN",
        style=discord.ButtonStyle.primary,
        custom_id=GROUP_FALLN_UNDER_18_BUTTON_ID,
    )
    async def falln(
        self,
        interaction: discord.Interaction,
        ...
        await finish_verification(interaction, FALLN_ROLE_ID, "FALLN", self.age_role_id, self.age_label)

    @discord.ui.button(
        label="F4LLN",
        style=discord.ButtonStyle.success,
        custom_id=GROUP_F4LLN_UNDER_18_BUTTON_ID,
    )
    async def f4lln(
        self,
        interaction: discord.Interaction,
        ...
        await finish_verification(interaction, F4LLN_ROLE_ID, "F4LLN", self.age_role_id, self.age_label)

    @discord.ui.button(
        label="FLL1N",
        style=discord.ButtonStyle.secondary,
        custom_id=GROUP_FLL1N_UNDER_18_BUTTON_ID,
    )
    async def fll1n(
        self,
        interaction: discord.Interaction,
        ...
        await finish_verification(
            interaction, FLL1N_ROLE_ID, "FLL1N", self.age_role_id, self.age_label
        )

    @discord.ui.button(
        label="Friendzone",
        style=discord.ButtonStyle.secondary,
        custom_id=GROUP_FRIENDZONE_UNDER_18_BUTTON_ID,
    )
    async def friendzone(
        self,
        interaction: discord.Interaction,
        button: discord.ui.Button,
    ) -> None:
        await finish_verification(
            interaction, FRIENDZONE_ROLE_ID, "Friendzone", self.age_role_id, self.age_label
        )
...
async def on_message(message: discord.Message) -> None:
    ...
    await cache_thread_screenshot(message)
...
@bot.tree.command(name="verahelp", description="Show Vera's command guide.")
async def verahelp(interaction: discord.Interaction) -> None:
    embed = discord.Embed(
        title="Vera's Little Help Desk",
        description=(
            "Hi, I am Vera, your handy verification bot.\n\n"
            "I keep the verification queue tidy, help the team make consistent decisions, "
            "and nudge verified players toward their next stop."
        ),
        color=VERA_PURPLE,
    )
    embed.add_field(
        name="/vera",
        value=(
            "Posts the welcome panel with the **Verify** button. "
            "Use this in the verification channel when the panel needs to be created again."
        ),
        inline=False,
    )
    embed.add_field(
        name="/verahelp",
        value="Shows this guide so everyone can see how I work.",
        inline=False,
    )
    embed.add_field(
        name="When Someone Clicks Verify",
        value=(
            "I open a private verification thread, ask them for a recent game profile screenshot, "
            "and ping Admin and Logistics so the team knows someone is waiting."
        ),
        inline=False,
    )
    embed.add_field(
        name="Staff Buttons",
        value=(
            "Admin or Logistics choose **Under 18** or **Over 18** first. "
            "Then I ask whether they should be **FALLN**, **F4LLN**, **FLL1N**, or **Friendzone**. "
            "Once chosen, I add the selected roles and remove waiting room."
        ),
        inline=False,
    )
    embed.add_field(
        name="Reject",
        value="Keeps the member in waiting room, logs the decision, and closes the thread after 30 seconds.",
        inline=False,
    )
    embed.add_field(
        name="Roles Reminder",
        value=(
            "When someone is verified, I tell them in the thread to go to the roles channel and I also try to DM "
            "them the roles-channel link. If Discord will not let me DM them, I say so in the thread so staff know."
        ),
        inline=False,
    )
    embed.add_field(
        name="/verarole",
        value=(
            "Lets Admin or Logistics add and remove server roles for a member without digging through the member list. "
            "You can add up to two roles and remove up to two roles in one go."
        ),
        inline=False,
    )
    embed.add_field(
        name="/verakick",
        value="Lets Admin or Logistics kick a member through Vera with an optional reason.",
        inline=False,
    )
    embed.add_field(
        name="Pulling Up Old Details",
        value=(
            "Staff can ask me to search swamp logs, but I need the wording to include the key phrase:\n"
            "`vera pull up [player name]'s verification details`\n\n"
            "Examples:\n"
            "`vera pull up Nell's verification details`\n"
            "`vera pull up 123456789012345678's verification details`\n\n"
            "I search server name, username, global name, and Discord user ID. The user ID is the most reliable."
        ),
        inline=False,
    )
...
@bot.tree.command(name="verarole", description="Add or remove roles for a member.")
@app_commands.describe(
    member="The member whose roles you want to update.",
    add_role="A role to add.",
    add_role_2="Another role to add.",
    remove_role="A role to remove.",
    remove_role_2="Another role to remove.",
...
async def verarole(
    ...
    add_role_2: discord.Role | None = None,
    remove_role: discord.Role | None = None,
    remove_role_2: discord.Role | None = None,
    reason: str | None = None,
) -> None:
    staff_member = await ensure_staff_member(interaction)
    if staff_member is None:
        return

    roles_to_add = unique_roles(add_role, add_role_2)
    roles_to_remove = unique_roles(remove_role, remove_role_2)

    if not roles_to_add and not roles_to_remove:
        await interaction.response.send_message(
            "Choose at least one role to add or remove.",
            ephemeral=True,
        )
        return

    if any(role.managed or role.is_default() for role in roles_to_add + roles_to_remove):
        await interaction.response.send_message(
            "I can't manage bot roles, integration roles, or the default @everyone role.",
            ephemeral=True,
        )
        return

    action_reason = reason or f"Role update by {staff_member}"
    ...
    try:
        ...
        await interaction.followup.send(
            "I couldn't update those roles. My role may be too low, or I may not have permission to manage one of them.",
            ephemeral=True,
        )
        return
    except discord.HTTPException:
        await interaction.followup.send(
            "Something went wrong while updating that member's roles.",
            ephemeral=True,
        )
        return
    ...
    embed = discord.Embed(
        title="Roles Updated",
        description=f"I've updated roles for {member.mention}.",
        color=VERA_PURPLE,
    )
    if added_names:
        embed.add_field(name="Added", value=", ".join(f"`{name}`" for name in added_names), inline=False)
    if removed_names:
        embed.add_field(name="Removed", value=", ".join(f"`{name}`" for name in removed_names), inline=False)
    embed.add_field(name="Updated by", value=display_name(staff_member), inline=False)
    if reason:
...
@bot.tree.command(name="verakick", description="Kick a member through Vera.")
@app_commands.describe(
...
async def verakick(
    ...
    if member == staff_member:
        await interaction.response.send_message(
            "I don't think kicking yourself is the move.",
            ephemeral=True,
        )
        return
    ...
    action_reason = reason or f"Kicked by {staff_member}"
    try:
        await member.kick(reason=action_reason)
    except discord.Forbidden:
        await interaction.followup.send(
            "I couldn't kick that member. I may need the Kick Members permission, or my role may be too low.",
            ephemeral=True,
        )
        return
    except discord.HTTPException:
        await interaction.followup.send(
            "Something went wrong while trying to kick that member.",
            ephemeral=True,
        )
        return
    ...
    embed = discord.Embed(
        title="Member Kicked",
        description=f"{member} has been removed from the server.",
        color=VERA_PURPLE,
    )
    embed.add_field(name="Actioned by", value=display_name(staff_member), inline=False)
    if reason:
        embed.add_field(name="Reason", value=reason, inline=False)
